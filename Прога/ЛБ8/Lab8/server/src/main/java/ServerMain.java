...
public class ServerMain {
    ...
    public static void main(String[] arg) throws IOException {
        ...
        while (true) {
            ...
            try {
                UDPServer.RawPacket rawPacket = udpServer.receiveRawData();

                if (rawPacket != null) {
                    //создаём поток, который будет распаковывать и анализировать запрос
                    new Thread(() -> {
                        try {
                            Request request = (Request) SerializationUtils.deserialize(rawPacket.data);

                            request.setClientAddress(rawPacket.address);
                            ...
                            logger.error("Ошибка при чтении (десериализации) запроса от {}: {}", rawPacket.address, e.getMessage());
                        }
                    }).start();
                }
            } catch (Exception e) {
