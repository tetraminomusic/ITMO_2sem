...
/**
 * API между программой и постгресом
 * Отвечает за подключение и закрытие подключения к БД. Есть отдельные методы, связанные с аутенфикацией.
 * Также класс может записывать объект в БД, обновлять и удалять его. Есть возможность регистрации пользователя
 */
