...
/**
 * Класс-ядро сервера.
 * Отвечает за физический обмен данными по UDP. Он может принимать запросы со стороны клиента, может отправлять ответ клиенту
 */
public class UDPServer {
    ...
    public static class RawPacket {
        ...
        public RawPacket(byte[] data, SocketAddress address) {
            this.data = data;
            this.address = address;
        }
    }

    public RawPacket receiveRawData() {
        try {
            ByteBuffer buffer = ByteBuffer.allocate(65536);
            SocketAddress clientAddress = channel.receive(buffer);

            if (clientAddress == null) {
                return null; // Пакетов нет (неблокирующий режим)
            }

            logger.info("Получен пакет (байты) от клиента: {}", clientAddress);
            ...
            logger.error("Ошибка при получении байтов из сети: {}", e.getMessage());
            return null;
        }
    }
