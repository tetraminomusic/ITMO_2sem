...
public class ExecuteScriptCommand{
     ...
     */
    public void execute(String path, String login, String password) {
        ...
        try (Scanner scriptScanner = new Scanner(file)) {
            ...
            while (scriptScanner.hasNextLine()) {
                ...
                if (command.equals("execute_script")) {
                    execute(argument, login, password); // Рекурсивный вызов этого же метода
                    continue;
                }
                ...
                if (objectCommands.contains(command)) {
                    ...
                    try {
                        ...
                        request = new Request(command, argument, lab, login, password);
                    } finally {
                        asker.setScriptScanner(null); // Возвращаем ввод на консоль
                    }
                } else {
                    request = new Request(command, argument, null, login, password);
                }
